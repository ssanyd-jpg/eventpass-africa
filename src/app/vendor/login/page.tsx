...
// No password — a Vendor isn't a User (see src/lib/vendor-auth.ts). "Event
// code" is the event's own public slug (the same identifier /events/[slug]
// already uses everywhere else), paired with the vendor's contact email to
// find the right Vendor row to email a magic link to. Always shows the same
// generic confirmation regardless of whether anything actually matched —
// see the request route for why.
export default function VendorLoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await fetch("/api/vendor/magic-link/request", {
        method: "POST",
  ...
  return (
    <div className="mx-auto flex min-h-[80vh] max-w-sm flex-col justify-center px-5 py-10">
      <h1 className="text-3xl font-bold">Vendor sign in</h1>
      <p className="mt-2 text-base text-muted">We&rsquo;ll email you a link to your sales dashboard — no password needed.</p>
